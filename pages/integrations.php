 ...
 * Gestion de claves API por usuario (iLovePDF, Gmail) + sistema (SMTP, Ghostscript para admin)
 */
...
$userApiRaw  = file_exists($userApiFile) ? (json_decode(file_get_contents($userApiFile), true) ?? []) : [];

// iLovePDF — por usuario
$hasPublic   = !empty($userApiRaw['ilp_public_key']);
$hasSecret   = !empty($userApiRaw['ilp_secret_key']);
$ilpConfigured = $hasPublic && $hasSecret;

// Gmail — por usuario
$gmailEmail     = $userApiRaw['gmail_email'] ?? '';
$gmailLabel     = $userApiRaw['gmail_label'] ?? 'Nexus';
$gmailLastSync  = $userApiRaw['gmail_last_sync'] ?? null;
$gmailHasPass   = !empty($userApiRaw['gmail_app_password']);
$gmailConfigured = !empty($gmailEmail) && $gmailHasPass;

// Admin: SMTP y Ghostscript (globales del sistema)
$smtpRaw       = [];
...
if ($isAdmin) {
    ...
    $gsQuality      = $smtpRaw['gs_quality'] ?? 'ebook';
    $gsPresets = [
...
<section id="panel-ilp" class="card integration-card integration-panel" role="tabpanel" aria-labelledby="tab-ilp">
    ...
    <form id="integrationsForm" class="integration-body" novalidate>
        ...
        <div class="integration-actions">
            ...
            <?php if ($ilpConfigured): ?>
            <button type="button" class="btn btn-subtle btn-danger-subtle" id="clearIlpBtn">
                <i class="bi bi-trash" aria-hidden="true"></i>
                <span class="btn-text"><?= __('integrations.btn_clear') ?></span>
            </button>
            <?php endif; ?>
